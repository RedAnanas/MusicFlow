...
const retryAppleMusic = async (id: string) => { try { await request(`/api/tasks/${id}/retry-apple-music`, { method: 'POST' }); ElMessage.success('Apple Music 交接已重新提交'); await store.fetchTasks() } catch (error) { ElMessage.error(error instanceof Error ? error.message : 'Apple Music 交接重试失败') } }
