...
<template>
  <div class="product-page system-page" v-loading="loading">
    ...
    <el-card v-if="status" class="system-card">
      ...
      <section class="tool musicdl-tool"><div class="musicdl-details"><h2>musicdl 服务</h2><p>配置 MusicFlow 可访问的 musicdl 地址，并检查连接与双库补齐接口。</p><div class="musicdl-form"><el-input v-model="musicdlUrl" placeholder="例如 http://musicdl-web:5000" aria-label="musicdl 服务地址"/><el-button :loading="savingMusicdl" @click="saveMusicdl">保存地址</el-button><el-button :loading="checkingMusicdl" @click="loadMusicdl">检测连接</el-button></div><small>Docker 容器中的 127.0.0.1 指向 MusicFlow 自身；请使用同网络的容器名或可访问的飞牛地址。</small><p v-if="musicdlStatus?.connected && !musicdlStatus.acquisition_supported" class="musicdl-warning">搜索和普通下载可连接，但当前 musicdl 缺少一键补齐所需的新版下载接口。</p><p v-else-if="musicdlStatus?.error" class="musicdl-warning">{{ musicdlStatus.error }}</p></div><el-tag v-if="musicdlStatus" :type="musicdlStatus.connected ? 'success' : 'danger'">{{ musicdlStatus.connected ? `已连接${musicdlStatus.version ? ` · ${musicdlStatus.version}` : ''}` : '未连接' }}</el-tag></section>
    </el-card>
  </div>
</template>
